...
def plan(home, content, mode=None):
    ...
    for target, value in changes.items():
        if target.is_symlink():
            if target.parent == home / '.config/systemd/user' and os.readlink(target) == '/dev/null':
                pass
            elif target.name == 'workstation-notifications.py' and 'dotfiles' in str(target.resolve()):
                pass
            else:
