...
def configure(home):
    ...
    for name in ('60-policykit-window.conf', '61-desktop-app-windows.conf', '62-system-utilities.conf'):
        ...
        if target.is_symlink() or (target.exists() and not target.read_text().startswith(content.splitlines()[0] + "\n")):
    ...
    if utility.is_symlink() or (utility.exists() and '# workstation-setup: managed system utility launcher' not in utility.read_text()):
        raise ValueError('Launcher utility personale da preservare: ' + str(utility))
    changes.append((utility, (ROOT / 'bin/workstation-system-tool').read_text(), True))
    ...
    if launcher.exists():
        ...
        if launcher.is_symlink() or not ('# workstation-setup: managed Cliamp launcher' in old or
        ...
        changes.append((launcher, (ROOT / 'bin/cliamp-widget').read_text(), True))
