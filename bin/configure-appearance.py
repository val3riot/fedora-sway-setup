...
def plan(home):
    ...
    for version in ('3.0', '4.0'):
        ...
        if path.is_symlink() and 'dotfiles' not in str(path.resolve()):
            raise ValueError('Preservare symlink personale: ' + str(path))
    ...
    theme_conf = ROOT / 'dotfiles/sway/.config/sway/config.d/99-theme.conf'
    if not theme_conf.exists():
        theme_conf = ROOT / 'templates/sway/config.d/99-theme.conf'
    managed = {
        ...
        '.config/sway/config.d/99-theme.conf': theme_conf.read_text(),
    }
    for name, content in managed.items():
        ...
        if path.is_symlink():
            if 'dotfiles' not in str(path.resolve()):
                raise ValueError('Preservare configurazione personale: ' + str(path))
        elif path.exists() and not path.read_text().startswith(content.splitlines()[0]+'\n') and path.read_text() != legacy_border:
            raise ValueError('Preservare configurazione personale: ' + str(path))
...
def apply(home):
    ...
    for path, content in changes:
        if path.exists() and path.read_text().strip() == content.strip(): continue
...
def doctor(home):
    ...
    for path, content in plan(home):
        if not path.exists() or path.read_text().strip() != content.strip(): errors.append(str(path))
    for key, value in INTERFACE.items():
